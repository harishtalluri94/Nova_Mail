...
async fn main() -> anyhow::Result<()> {
    ...
    logging::init_logging("previewer", Some(&observability_config.otlp_endpoint))?;

    let _guard = observability_config
        .sentry_dsn
        .as_ref()
        .map(|dsn| logging::init_sentry(dsn, "previewer"));

    tracing::info!("Starting Nova Mail Previewer v{}", VERSION);
    ...
    for i in 0..4 {
        ...
        tokio::spawn(async move {
            tracing::info!("Starting preview worker {}", i);
    ...
    tracing::info!("Previewer health server listening on {}", addr);
    ...
    tracing::info!("Previewer shutdown complete");
    Ok(())
}
